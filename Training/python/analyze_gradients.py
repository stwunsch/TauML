...
def plot_concat_mean(x):
    plt.figure(figsize=(4, 4))
    for i, obj in enumerate(objs):
        plt.plot(range(3), [np.mean(x[i][:57]), np.mean(x[i][57:57+64]), np.mean(x[i][57+64:])], "o-", alpha=0.7, label=obj)
    plt.xlabel("Averaged nodes of the concat layer")
    plt.ylabel("Mean abs. of the gradient")
    plt.legend()
    plt.xlim((-1, 3))
    ...
    plt.tight_layout()
    plt.savefig("grad_concat_mean_" + os.path.basename(file_name).replace(".h5", "") + ".png")

plot_concat(concat_grads)
plot_concat_mean(concat_grads)
