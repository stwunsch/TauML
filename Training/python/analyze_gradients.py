...
class Predictor:
    ...
    def Predict(self, session, X):
        ...
        pred, grad_e, grad_mu, grad_tau, grad_jet = session.run([self.y_graph, self.grad_e, self.grad_mu, self.grad_tau, self.grad_jet], feed_dict=feed_dict)
        ...
        return pandas.DataFrame(data = {
            ...
            'deepId_jet': pred[:, jet]
        }), np.sum(np.abs(grad_e), axis=0), np.sum(np.abs(grad_mu), axis=0), np.sum(np.abs(grad_tau), axis=0), np.sum(np.abs(grad_jet), axis=0)
...
for file_name in file_list:
    ...
    with tqdm(total=loader.data_size, unit='taus') as pbar:
        for inputs in loader.generator(return_truth = False, return_weights = False):
            df, grad_e_, grad_mu_, grad_tau_, grad_jet_ = predictor.Predict(sess, inputs)
            grad_e += grad_e_
            grad_mu += grad_mu_
            grad_tau += grad_tau_
            grad_jet += grad_jet_
...
plt.xlabel("Nodes of concat. layer")
